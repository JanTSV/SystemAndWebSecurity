...
The output of the code is: \texttt{undvier}.


When we try to \verb|strcpy foo| into \verb|buffer3| we get an overflow, because the \verb|string| in \verb|foo| is \verb|"zweiundvier\0zig"|.
\verb|foo| gets cut at the null-termination so what we actually try to copy into \verb|buffer3| is \texttt{zweiundvier}.
When we look at \verb|buffer3[4]| we can see that it can take 4 bytes of the \verb|string|. 
So [0] = z, [1] = w, [2] = e, [3] = i.
Now the overflow happens and writes the rest of the string into the memory of \verb|buffer2[3]| and \verb|buffer1[8]|.
If we would also print \verb|buffer1| we only get the output \texttt{vier}.
...
\stackVariable{C1C}{c}
\stackVariable{C20}{d}
\stackVariable{C24}{oldFrameP}
\stackVariable{C28}{RetAddr}
\stackPointerToAddr{C24}{ebp}
\stackPointerToAddr{C1C}{esp}
\stackVariable{C2C}{a}
\stackVariable{C30}{b}
...
\stackLine[24]{C34}
\stackVariable{C34}{globOfftable}
\stackVariable{C38}{e}
\stackVariable{C3C}{f}
\stackVariable{C40}{g}
\stackVariable{C44}{oldFrameP}
\stackVariable{C48}{RetAddr}
